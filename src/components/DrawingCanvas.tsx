...
import { Stage, Layer, Line, Rect, Circle, Group, Text, Label, Tag } from "react-konva";
import { Html } from "react-konva-utils";
import { useStore, DrawingElement, Point } from "../store/useStore";
  ...
  return (
    ...
    >
        ...
        <Layer>
          {visibleElements.map((el) => {
            ...
            if (
              ...
              const baseSize = 8 + (el.diameter || 2) * 2;
              const isLocked = isLayerLocked(el.layerId);
              return (
                <Group key={el.id}>
                  {el.type === "support" && (
          ...
          {newPipe && (
            <Line
              points={newPipe}
              stroke="#4dabf7"
              strokeWidth={4 + currentDiameter * 2}
              opacity={0.5}
              lineCap="round"
            />
          )}
      ...
      {notification && (
        <div
          className={cn(
            "absolute top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-6 py-3 rounded-2xl border shadow-2xl backdrop-blur-md animate-in slide-in-from-top-4 duration-300",
