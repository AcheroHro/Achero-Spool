...
export type AccessoryType = 'elbow' | 'tee' | 'flange' | 'valve' | 'reducer';
...
export const useStore = create<AppState>((set, get) => ({
  ...
  clearDrawing: () => set({ 
    ...
    historyIndex: -1, 
    selectedId: null,
    layers: [{ id: DEFAULT_LAYER_ID, name: 'Capa Base', visible: true, locked: false }],
    activeLayerId: DEFAULT_LAYER_ID
  }),
  ...
  setDrawing: (state) => set({ 
    elements: state.elements || [], 
    selectedId: null,
    viewPos: state.viewPos || { x: 0, y: 0 }, 
    scale: state.scale || 1,
    layers: state.layers || [{ id: DEFAULT_LAYER_ID, name: 'Capa Base', visible: true, locked: false }],
